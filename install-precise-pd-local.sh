...
helm upgrade --install "gaie-${RELEASE_NAME_POSTFIX}" "${SCRIPT_DIR}/charts/inferencepool" \
  --namespace "${NAMESPACE}" \
  --values "${SCRIPT_DIR}/charts/inferencepool/values-snd-pd.yaml" \
...
helm upgrade --install "ms-${RELEASE_NAME_POSTFIX}" "${SCRIPT_DIR}/charts/llm-d-modelservice" \
  --namespace "${NAMESPACE}" \
  --values "${SCRIPT_DIR}/charts/llm-d-modelservice/values-snd-pd.yaml" \
  --set-string "decode.containers[0].env[0].value=${RELEASE_NAME_POSTFIX}" \
  --set-string "prefill.containers[0].env[0].value=${RELEASE_NAME_POSTFIX}" \
  --wait
